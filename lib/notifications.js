  ...
  tempReminderObservable((reminder) => {
    ...
    if (reminder.enabled) {
      ...
      PushNotification.localNotificationSchedule({
        ...
        message: i18next.t(
          'hamburgerMenu.settings.reminders.tempReminder.notification'
        ),
        date: target.toDate(),
        vibrate: false,
        repeatType: 'day',
  ...
  if (bleedingPrediction.length > 0) {
    ...
    if (reminderDate.isAfter()) {
      ...
      PushNotification.localNotificationSchedule({
        ...
        message: i18next.t(
          'hamburgerMenu.settings.reminders.periodReminder.notification',
          {
            advanceNoticeDays: advanceNoticeDays,
            daysToEndOfPrediction: daysToEndOfPrediction,
          }
        ),
