  ...
  const getStats = () =>
    cycleStartsSortedByDate.map((day, i) => {
      ...
      if (i === 0) {
        ...
        // for all other cylces, subtract the previous start date from the current start date
        const parsedDay = LocalDate.parse(day.date)
        const parsedPreviousCycleStartDay = LocalDate.parse(cycleStartsSortedByDate[i - 1].date)
