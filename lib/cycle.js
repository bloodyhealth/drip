...
export default function config(opts) {
  ...
  const computeNextStatsBatch = (start, end) => {
    const cycleStartsArray = Array.from(cycleStartsSortedByDate)
    end > cycleDaysSortedByDate.length
      ? (end = cycleDaysSortedByDate.length)
      : end

    return cycleStartsArray.slice(start, end).map((day, i) => {
      let cycleLength
      if (i + start === 0) {
        // start is an offset which translates the index of the sliced array back into the value range of the original database object
        // in the most recent cycle, its length equals the number of the current day within the cycle
        cycleLength = getCycleDayNumber(getTodayDate())
      } else {
        // for all other cycles, subtract the previous start date from the current start date
        const parsedDay = LocalDate.parse(day.date)
        const parsedPreviousCycleStartDay = LocalDate.parse(
          cycleStartsSortedByDate[i + start - 1].date
        )
        cycleLength = parsedDay.until(parsedPreviousCycleStartDay, DAYS)
      }
      return {
        date: day.date,
        cycleLength,
