...
function validateHeaders(headers) {
  ...
  if (
    ...
    throw new Error(
      i18next.t('sideMenu.settings.data.import.error.incorrectColumns', {
  ...
  for (const i in cycleDays) {
    ...
    if (
      ...
      Object.keys(day).some(
        (symptom) => symptom !== 'date' && symptom !== 'note'
      )
    ) {
      throw new Error(
        i18next.t('sideMenu.settings.data.import.error.futureEdit')
      )
    }
  }
}
