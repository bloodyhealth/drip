...
export default async function importCsv(csv, deleteFirst, t) {
  const parseFuncs = {
    bool: (val) => {
...
function validateHeaders(headers, t) {
  ...
  if (
    ...
    throw new Error(
      t('error.incorrectColumns', { incorrectColumns: expectedHeaders.join() })
    )
  }
}
...
function throwIfFutureData(cycleDays, t) {
  ...
  for (const i in cycleDays) {
    ...
    if (
      ...
      throw new Error(t('error.futureEdit'))
    }
  }
}
